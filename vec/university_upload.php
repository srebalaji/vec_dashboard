...
//displaying values from spreadsheet
    $data = new Spreadsheet_Excel_Reader("uploads/university_upload.xls");
...
{	
	if(count($data->sheets[$i]["cells"])>0) // checking sheet not empty
	{
		...
		{ 
			...
			//$add.="".$one."";
			$add2.="rollno=values(rollno),subject=values(subject),marks=values(marks)";
		...
		//echo $add;
		//echo $one;
		$query="insert into ".$tablename."(rollno,subject,marks)".$one." on duplicate key update ".$add2;
		//echo $query;
