...
<div class="content-wrapper">
  ...
  <form action="ptexams1_upload.php" method="post" enctype="multipart/form-data">
...
<div class="row">
	...
	<form method="get" action="pt_results.php">
...
if($query=mysql_query($using_again))
{
   ...
   echo '</tbody></table>
	...
	echo '<input type="submit" value="update Pt-1 results" class="btn btn-primary"/>';
     echo '</form>';
...
if($query=mysql_query($using_again))
{
	...
	echo '<input type="submit" value="update PT-1 marks" class="btn btn-primary"/></form>';
...
if(isset($_GET['subjects']) && isset($_GET['marks']))
{
	...
	//echo '<script>$("body").html("<center><div style=\"color:#2ecc71;font-size:24px\">"+up+"</div></center>");</script>';
	 echo "<script>window.location.href='pt_results.php?years=".$years."';</script>";
}
?>
...
else {echo '<center><h3>Year is not selected properly...go back to selection pagess </br> <a href="cate.php">Selection page</a></h3></center>';  }
}
else
{
