...
//displaying values from spreadsheet
    $data = new Spreadsheet_Excel_Reader("uploads/master.xls");
...
{	
	if(count($data->sheets[$i]["cells"])>0) // checking sheet not empty
	{
		...
		{ 
			...
			$one= $data->sheets[$i]['cells'][$j][1];
			$two=$data->sheets[$i]['cells'][$j][2];
			$add.="(".$one.",'".$name."',".$two.")";
			$add2.="rollno=values(rollno), dept=values(dept), year=values(year)";
		...
		//echo $add;
		$query="insert into ".$tablename."(rollno,dept,year)values".$add." on duplicate key update ".$add2;
		//echo $query;
