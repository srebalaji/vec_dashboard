...
//displaying values from spreadsheet
    $data = new Spreadsheet_Excel_Reader("uploads/attendance3.xls");
...
{	
	if(count($data->sheets[$i]["cells"])>0) // checking sheet not empty
	{
		...
		{ 
			...
			$add.="".$one."";
			$add2.="rollno=values(rollno), atten_3=values(atten_3)";
		...
		//echo $add;
		$query="insert into ".$tablename."(rollno,atten_3)".$add." on duplicate key update ".$add2;
		//echo $query;
