...
//displaying values from spreadsheet
    $data = new Spreadsheet_Excel_Reader("uploads/attendance2.xls");
...
{	
	if(count($data->sheets[$i]["cells"])>0) // checking sheet not empty
	{
		...
		{ 
			...
			$add.="".$one."";
			$add2.="rollno=values(rollno), atten_2=values(atten_2)";
		...
		//echo $add;
		$query="insert into ".$tablename."(rollno,atten_2)".$add." on duplicate key update ".$add2;
		//echo $query;
