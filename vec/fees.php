...
$('.plusbtn').click(function(){
	click++;
$('#two').append('<th><input type="text" name="subjects[]" size="20" placeholder="Title"/></th>');

$('.three').append(' <td><input type="text" name="marks[]" size="20" placeholder="Description"/></td>');
...
$marks=array();
$ids=array();
$using_again="select $table_name.id,$table_name.rollno,$table_name.dept,$table_name1.subject,$table_name1.marks from $table_name left join $table_name1 on $table_name.id=$table_name1.rollno where $table_name.year=".$years;
?>
...
if($data['ret']==0)
{
	?>
	<thead>
<tr id="two"><th style="width:20%">Register Number</th><th><input type="text" name="subjects[]" size="20" placeholder="Title"/></th><th><input type="text" name="subjects[]" size="20" placeholder="Title"/></th><th><input type="text" name="subjects[]" size="20" placeholder="Title"/></th></tr>
</thead>
	<?php
if($query=mysql_query($using_again))
{
	...
	while($fetch=mysql_fetch_assoc($query))
	{
       //echo $fetch['rollno'].'</br>'.$fetch['year'];
		...
		?>
		<tbody>
         <tr class="three"><td><?php echo $rollno;echo '<input type="hidden" value="'.$id.'" name="ids[]"'; ?></td><td><input type="text" name="marks[]"  size="25" placeholder="Description" /></td>
         <td><input type="text" name="marks[]" size="25" placeholder="Description"/></td>
         <td><input type="text" name="marks[]" size="25" placeholder="Description"/></td>
        </tr>
     </tbody>
    <?php
	...
	*/
	...
	echo '<input type="submit" value="update fees details" class="btn btn-primary"/>';
     echo '</form>';
...
if($query=mysql_query($using_again))
{
	...
	while($inc!=$countsix)
	{?>
       <th><input type="text" name="subjects[]" size="20" placeholder="Title" value="<?php if(isset($subject[$inc]) && !empty($subject[$inc]))  echo $subject[$inc]; ?>"/></th>
       <?
       $inc++;
	}
	echo '</tr>';
	...
	{
		if($subcount>0){?>
       <td><input type="text" name="marks[]" size="25" placeholder="Description" value="<?php if(isset($mark[$j]) && !empty($mark[$j]))  echo $mark[$j]; ?>"/></td>
       <?}
       else{?>
       <td><input type="text" name="marks[]" size="25" placeholder="Description" value=""/></td>
	...
	echo '<input type="submit" value="update fees details" class="btn btn-primary"/></form>';
...
if(isset($_GET['subjects']) && isset($_GET['marks']))
{
	...
	for($i=0;$i<$numof;$i++)
	{
		...
		/*if(is_null($extra))
			...
			$main='insert into '.$table_name1.'(rollno,subject,marks) values '.implode(',',$sql);
		
		else*/
		$main='insert into '.$table_name1.'(rollno,subject,marks) values '.implode(',',$sql).' on duplicate key update '.implode(',',$extra);
		//echo $main;
	...
	//echo '<script>$("body").html("<center><div style=\"color:#2ecc71;font-size:24px\">"+up+"</div></center>");</script>';
	 echo "<script>window.location.href='fees.php?years=".$years."';</script>";
}
?>
