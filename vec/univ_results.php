...
$table_name1=$name.'_univ_results';
?>
<!DOCTYPE html>
<html>
  <head>
...
?>
 ...
 <div class="content-wrapper">
  <section class="content">
   <form action="university_upload.php" method="post" enctype="multipart/form-data">
...
<div class="row">
	...
	<form method="get" action="univ_results.php">
...
if($qq=mysql_query("select count(*) as ret from $table_name1 a left join $table_name b on a.rollno=b.id where year='$years'"))
  //select count(*) as ret from mech_univ_results left join mech_master on mech_univ_results.rollno=mech_master.id where year=3
...
if($query=mysql_query($using_again))
{
   ...
   echo '</tbody></table>
	...
	echo '<input type="submit" value="update university results" class="btn btn-primary"/>';
     echo '</form>';
...
if($query=mysql_query($using_again))
{
	...
	echo '<input type="submit" value="update university marks" class="btn btn-primary"/></form>';
...
if(isset($_GET['subjects']) && isset($_GET['marks']))
{
	...
	//echo '<script>$("body").html("<center><div style=\"color:#2ecc71;font-size:24px\">"+up+"</div></center>");</script>';
	 echo "<script>window.location.href='univ_results.php?years=".$years."';</script>";
}
?>
</body>
</html>
