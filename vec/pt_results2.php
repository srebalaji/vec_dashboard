...
$marks=array();
$ids=array();
$using_again="select $table_name.id,$table_name.rollno,$table_name.dept,$table_name1.subject,$table_name1.marks2 from $table_name left join $table_name1 on $table_name.id=$table_name1.rollno where $table_name.year=".$years;
?>
 
  <div class="content-wrapper">
  <section class="content">
   <form action="ptexams2_upload.php" method="post" enctype="multipart/form-data">
...
<div class="row">
	...
	<form method="get" action="pt_results2.php">
...
if($query=mysql_query($using_again))
{
   ...
   echo '</tbody></table>
	...
	echo '<input type="submit" value="update Pt-2 results" class="btn btn-primary"/>';
     echo '</form>';
...
if($query=mysql_query($using_again))
{
	...
	echo '<input type="submit" value="update PT-2 marks" class="btn btn-primary"/></form>';
...
if(isset($_GET['subjects']) && isset($_GET['marks']))
{
	...
	for($i=0;$i<$numof;$i++)
	{
		...
		while($sub_count)
		{
		  ...
		  $extra[]='rollno=values(rollno), subject=values(subject), marks2=values(marks2)';
		  $j++;$sub_count--;$sub_inc++;
		...
		$main='insert into '.$table_name1.'(rollno,subject,marks2) values '.implode(',',$sql).' on duplicate key update '.implode(',',$extra);
		//echo $main;
	...
	//echo '<script>$("body").html("<center><div style=\"color:#2ecc71;font-size:24px\">"+up+"</div></center>");</script>';
	 echo "<script>window.location.href='pt_results2.php?years=".$years."';</script>";
}
?>
